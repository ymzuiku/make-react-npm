...
# bash deploy.sh root@120.120.120.120 deploy 0.0.1 #部署为 0.0.1 版本
# bash deploy.sh root@120.120.120.120 rollback 0.0.2 #回滚至 0.0.2 版本
...
if [ "$2" == "" ];then
  ...
  echo "like:"
  echo "bash deploy.sh root@120.120.120.120 deploy #部署，并且使用package.json上的版本号做备份，以待回滚"
  echo "bash deploy.sh root@120.120.120.120 rollback 0.0.2 #回滚至v002版本"
  echo " "
  exit 0
fi
